...
namespace PracticeAPI_Test
{
    ...
    public class Practice_API_Test
    {
        ...
        [Fact]
        public async Task Get_ID_In_FoodDistribution_But_Exist_In_Animal_And_Food_ShouldBe()
        {
            //Arrange
            var id = Guid.NewGuid();
           // var animal = new Animal(id,"Hulk","cat",false);
            
           
        
            var response = await _client.GetAsync($"Animal/Get/{id}");
            var responseText = await response.Content.ReadAsStringAsync();
            //var result = JsonSerializer.Deserialize<Animal>(responseText);
